...
public class MuSigCreateOfferPriceController implements Controller {
    ...
    public void onActivate() {
        ...
        priceInput.setDescription(Res.get("muSig.price.tradePrice.inputBoxText", marketCodes));
    ...
    void onPercentageFocused(boolean focused) {
        ...
        if (!focused) {
            try {
                ...
                model.getErrorMessage().set(Res.get("muSig.price.warn.invalidPrice.numberFormatException"));
            }
        }
    }
    ...
    private void applyPercentageString(String percentageAsString) {
        ...
        try {
            ...
            model.getErrorMessage().set(Res.get("muSig.price.warn.invalidPrice.exception", e.getMessage()));
        }
    }
    ...
    private boolean validatePercentage(double percentage) {
        if (percentage >= model.getMinPercentage() && percentage <= model.getMaxPercentage()) {
            ...
            model.getErrorMessage().set(Res.get("muSig.price.warn.invalidPrice.outOfRange"));
            return false;
        }
    }
    ...
    private void updateFeedback(PriceSpec priceSpec) {
        ...
        if (percentage.isPresent()) {
            ...
            if (percentageValue < -0.05) {
                feedbackSentence = getFeedbackSentence(Res.get("muSig.price.feedback.sentence.veryLow"));
            } else if (percentageValue < 0d) {
                feedbackSentence = getFeedbackSentence(Res.get("muSig.price.feedback.sentence.low"));
            } else if (percentageValue < 0.05) {
                feedbackSentence = getFeedbackSentence(Res.get("muSig.price.feedback.sentence.some"));
            } else if (percentageValue < 0.15) {
                feedbackSentence = getFeedbackSentence(Res.get("muSig.price.feedback.sentence.good"));
            } else {
                feedbackSentence = getFeedbackSentence(Res.get("muSig.price.feedback.sentence.veryGood"));
            }
    ...
    private String getFeedbackSentence(String adjective) {
        return model.getDisplayDirection().isBuy()
                ? Res.get("muSig.price.feedback.buyOffer.sentence", adjective)
                : Res.get("muSig.price.feedback.sellOffer.sentence", adjective);
    }
