...
import bisq.desktop.main.content.mu_sig.trade.open_trades.trade_state.MuSigTradeDataHeader;
import bisq.desktop.main.content.mu_sig.trade.open_trades.trade_state.MuSigTradeStateController;
...
public final class MuSigOpenTradesController extends ChatController<MuSigOpenTradesView, MuSigOpenTradesModel> {
    ...
    public void createDependencies(ChatChannelDomain chatChannelDomain) {
        ...
        muSigTradeDataHeader = new MuSigTradeDataHeader(serviceProvider, Res.get("muSig.openTrades.chat.peer.description").toUpperCase());
    }
    ...
    protected void selectedChannelChanged(ChatChannel<? extends ChatMessage> chatChannel) {
        ...
        UIThread.run(() -> {
            if (!hasTradeForChannel(chatChannel)) {
                ...
                model.getChatWindowTitle().set(Res.get("muSig.openTrades.chat.window.title",
